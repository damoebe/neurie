 ...
 */
public class DeepNetwork extends FFNetwork {
    ...
    private void updateHiddenDeltas() {
        for (int l = layers.size() - 2; l >= 0; l--) { // skip output and input layer
            for (Neuron neuron : layers.get(l).neurons()) {
                ...
                double delta;
                if (l == 0){
                    delta = sum; // only sum because input is not put into the sigmoid function
                }else{
                    delta = output * (1 - output) * sum; // derivative of sigmoid * sum
                }
