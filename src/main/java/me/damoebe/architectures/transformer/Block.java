...
import me.damoebe.architectures.transformer.mha.EDHead;
 ...
 */
public class Block <H extends Head, N extends FFNetwork>{
     ...
     * @param multiLayerPerceptron A FFNetwork object that will be cloned and used as an MLP architecture.
     */
    ...
    public Sequence getOutputFor(Sequence... inputEmbeddings){
        Sequence mhaOutput = multiHeadAttention.generateOutputFor(preMHANorm.normalize(inputEmbeddings));
    ...
    public Block<H, N> clone(){
        return new Block<>(multiHeadAttention, multiLayerPerceptrons.getFirst());
    }
}
